...
void DDraw::DirectDrawEnumerateA(uint32_t callback, uint32_t context)
{
    ...
    uc_mem_write(uc, ptr, driver_desc, strlen(driver_desc));
    uc_mem_write(uc, ptr+strlen(driver_desc)+1, driver_name, strlen(driver_name));
    ...
    pc_over = true;
    uc_reg_write(uc, UC_X86_REG_EIP, &callback); //TODO this isn't working...
}
