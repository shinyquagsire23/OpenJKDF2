...
// Expose the SAHD-declared audio rate (DroidWorks .san files declare
...
// Channel count from the IACT track flags (see smush_proc_iact).
...
void smush_proc_fobj(smush_ctx* ctx, uint32_t seek_pos, uint32_t total_size)
{
    ...
    if (fobj.codec == 48) {
        ...
        // Once-per-codec latch — unsupported codecs would otherwise print this per FOBJ.
