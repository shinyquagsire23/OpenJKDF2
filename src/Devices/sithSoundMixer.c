...
sithPlayingSound* sithSoundMixer_PlayingSoundFromSound(sithSound *sound, int flags)
{
    ...
    result->pitch = 1.0;
    if ( sithSoundMixer_channelGUIDSeed == 0 )
        sithSoundMixer_channelGUIDSeed = 1;
    result->refid = sithSoundMixer_channelGUIDSeed++ | ((playerThingIdx + 1) << 16);
    return result;
}
...
void sithSoundMixer_TickSectorSound()
{
    ...
    {
        v4 = -sithSoundMixer_hCurAmbientChannel->vol_2;
        if (v4 == 0.0)
        {
            sithSoundMixer_hCurAmbientChannel->flags |= SITHSOUNDFLAG_FADING;
    ...
    if (sithSoundMixer_hCurAmbientChannel->sound == lastSectorFadingOutSound)
    {
        ...
        sithSoundMixer_hCurAmbientChannel->flags &= ~(SITHSOUNDFLAG_FADE_OUT|SITHSOUNDFLAG_FADE_IN|SITHSOUNDFLAG_FADING);
    }
    else
    {
        v20 = -sithSoundMixer_hCurAmbientChannel->vol_2;
        if (v20 != 0.0)
        {
            sithSoundMixer_hCurAmbientChannel->flags &= ~(SITHSOUNDFLAG_FADE_OUT|SITHSOUNDFLAG_FADE_IN|SITHSOUNDFLAG_FADING);
...
void sithSoundMixer_Update(flex_t deltaSecs)
{
    rdVector3 tmp;
    ...
    //printf("--- %u\n", sithSoundMixer_activeChannels);
    rdVector_Scale3(&tmp, &sithCamera_g_pCurCamera->lookPos, 10.0);
    stdSound_SetPositionOrientation(&tmp, &sithCamera_g_pCurCamera->orient.lvec, &sithCamera_g_pCurCamera->orient.uvec);
}
...
void sithSoundMixer_UpdateSoundPos(sithPlayingSound *sound)
{
    ...
    else
    {
        ...
        {
            v5 = sound->pSoundBuf;
            a1a = rdVector_Dot3(&sithCamera_g_pCurCamera->orient.lvec, &sound->posRelative);
            a2 = rdVector_Dot3(&sithCamera_g_pCurCamera->orient.rvec, &sound->posRelative);
...
void sithSoundMixer_SyncSounds()
{
    for (int i = 0; i < SITH_MIXER_NUMPLAYINGSOUNDS; i++)
    {
        ...
        if (iter != sithSoundMixer_hCurAmbientChannel)
        {
...
void sithSoundMixer_UpdatePlayingSoundPosition(sithPlayingSound *sound)
{
    ...
    {
        rdVector_Sub3(&sound->posRelative, &sound->pos, &sithCamera_g_pCurCamera->lookPos);
    }
    else
    {
